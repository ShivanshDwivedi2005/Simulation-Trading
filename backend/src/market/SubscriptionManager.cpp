...
SubscriptionManager::SubscriptionManager(
  ...
  if (pinned_symbols.size() != 5)
    throw std::runtime_error("exactly five pinned symbols are required");

  std::set<std::string> unique;
  for (const auto &raw_symbol : pinned_symbols) {
    const auto symbol = normalize_symbol(raw_symbol);
    if (!valid_symbol(symbol))
      throw std::runtime_error("invalid pinned symbol syntax: " + raw_symbol);
    if (!unique.insert(symbol).second)
      throw std::runtime_error("duplicate pinned symbol: " + symbol);
    if (!validator_ || !validator_(symbol)) {
      throw std::runtime_error(
          "pinned symbol is not present in the Alpaca instrument catalogue: " +
          symbol);
    }
  }
  ...
  try {
    invoke_void([this, pinned_symbols = std::move(pinned_symbols)] {
      for (const auto &raw_symbol : pinned_symbols) {
        ...
        if (!begin_subscribe(state))
          throw std::runtime_error("failed to reserve pinned symbol: " +
                                   symbol);
      }
    });
  } catch (...) {
...
WatchResult SubscriptionManager::watch(const std::string &symbol) {
  return invoke<WatchResult>(
      [this, symbol] { return acquire_impl(symbol, false); });
}
...
WatchResult SubscriptionManager::protect_order(const std::string &symbol) {
  return invoke<WatchResult>(
      [this, symbol] { return acquire_impl(symbol, true); });
}
...
void SubscriptionManager::restore_pending_orders(
  ...
  invoke_void([this, counts] {
    for (const auto &[raw_symbol, count] : counts) {
      ...
      if (!valid_symbol(symbol) || !validator_ || !validator_(symbol)) {
        throw std::runtime_error(
            "persisted pending order references an invalid catalogue symbol: " +
            raw_symbol);
      }
      auto &state = states_[symbol];
      state.symbol = symbol;
      ...
      state.last_requested_at = clock_();
      if (!state.subscribed && !state.subscribe_pending)
...
void SubscriptionManager::on_confirmation(
    std::set<std::string> confirmed_symbols) {
...
void SubscriptionManager::set_status_handler(StatusHandler handler) {
  invoke_void([this, handler = std::move(handler)]() mutable {
...
std::vector<QueuedSymbolSnapshot> SubscriptionManager::queue_snapshot() const {
  return invoke<std::vector<QueuedSymbolSnapshot>>([this] {
    ...
    for (std::size_t index = 0; index < ordered.size(); ++index) {
      ...
      result.push_back(QueuedSymbolSnapshot{
          queued.symbol, index + 1, queued.viewer_demand,
          queued.pending_order_demand,
          std::chrono::duration_cast<std::chrono::milliseconds>(
              now - queued.queued_at),
...
std::optional<std::string> SubscriptionManager::eviction_candidate() const {
  return invoke<std::optional<std::string>>(
      [this] { return eviction_candidate_impl(); });
}

std::size_t SubscriptionManager::active_symbol_count() const {
  return invoke<std::size_t>(
      [this] { return active_subscription_count_impl(); });
}
...
void SubscriptionManager::enqueue(Task task) const {
  {
    ...
    if (stopping_)
      throw std::runtime_error("subscription manager is stopping");
...
void SubscriptionManager::run() {
  for (;;) {
    Task task;
    {
      ...
      queue_condition_.wait_for(lock, kQueuePollInterval, [this] {
...
WatchResult SubscriptionManager::acquire_impl(const std::string &raw_symbol,
                                              bool for_order) {
  const auto symbol = normalize_symbol(raw_symbol);
  if (!valid_symbol(symbol) || !validator_ || !validator_(symbol)) {
  ...
  if (state.subscribe_pending ||
      (transition_target_ && *transition_target_ == symbol)) {
...
void SubscriptionManager::release_impl(const std::string &raw_symbol,
                                       bool for_order) {
  const auto symbol = normalize_symbol(raw_symbol);
  const auto found = states_.find(symbol);
  if (found == states_.end())
  ...
  if (state.viewer_count == 0 && state.pending_order_count == 0)
    state.last_viewer_left_at = clock_();
...
bool SubscriptionManager::begin_subscribe(SymbolSubscription &state) {
  if (state.subscribe_pending || state.subscribed || state.unsubscribe_pending)
    return true;
  state.subscribe_pending = true;
  const auto result = upstream_.subscribe(state.symbol);
  if (result == SubscriptionResult::added ||
      result == SubscriptionResult::duplicate) {
    ++transition_count_;
    return true;
  }
...
bool SubscriptionManager::queue_needed(const SymbolSubscription &state) const {
  return state.pinned || state.viewer_count > 0 ||
         state.pending_order_count > 0;
}
...
void SubscriptionManager::update_waiting_entry(SymbolSubscription &state) {
  ...
  const auto [iterator, inserted] = waiting_.try_emplace(
      state.symbol, QueuedSymbol{state.symbol, clock_(), state.viewer_count,
                                 state.pending_order_count});
  if (!inserted) {
...
void SubscriptionManager::process_queue_impl() {
  ...
  if (transition_target_ || eviction_victim_ || waiting_.empty())
    return;
  const auto ordered = ordered_queue_symbols_impl();
  if (ordered.empty())
    return;
  ...
  if (active_subscription_count_impl() < capacity_) {
    if (begin_subscribe(target)) {
      waiting_.erase(target_symbol);
      notify_status(target_symbol, "CONNECTING", false, std::nullopt,
                    "Connecting to live market data.");
      process_queue_impl();
      return;
    }
    notify_status(target_symbol, "ERROR", false,
                  queue_position_impl(target_symbol),
  ...
  ++transition_count_;
  notify_status(target_symbol, "CONNECTING", false, std::nullopt,
                "Preparing a live market-data slot.");
}

std::vector<std::string>
SubscriptionManager::ordered_queue_symbols_impl() const {
  ...
  std::sort(ordered.begin(), ordered.end(),
            [](const QueuedSymbol *left, const QueuedSymbol *right) {
              const bool left_order = left->pending_order_demand > 0;
              const bool right_order = right->pending_order_demand > 0;
              if (left_order != right_order)
                return left_order > right_order;
              if (left->viewer_demand != right->viewer_demand)
                return left->viewer_demand > right->viewer_demand;
              if (left->queued_at != right->queued_at)
  ...
  result.reserve(ordered.size());
  for (const auto *queued : ordered)
    result.push_back(queued->symbol);
  return result;
}

std::optional<std::size_t>
SubscriptionManager::queue_position_impl(const std::string &symbol) const {
  const auto ordered = ordered_queue_symbols_impl();
...
void SubscriptionManager::notify_queue_positions() {
  ...
  for (std::size_t index = 0; index < ordered.size(); ++index) {
    ...
    if (transition_target_ && *transition_target_ == symbol)
      continue;
    notify_status(symbol, "QUEUED", false, index + 1,
                  "Live data is currently at capacity. This stock is queued "
                  "and the latest available snapshot is being shown.");
  }
}

void SubscriptionManager::handle_subscription_confirmation_impl(
    const std::set<std::string> &confirmed_symbols) {
  const auto now = clock_();
  ...
  for (auto &[symbol, state] : states_) {
    ...
    if (confirmed) {
      if (!state.subscribed)
        state.subscribed_at = now;
      state.subscribed = true;
      if (state.subscribe_pending)
        state.subscribe_pending = false;
      if (was_subscribe_pending)
        newly_live.insert(symbol);
  ...
  if (eviction_victim_) {
    ...
    if (victim == states_.end() || !victim->second.unsubscribe_pending) {
      ...
      if (transition_target_) {
        ...
        if (queue_needed(target)) {
          if (begin_subscribe(target)) {
            notify_status(target.symbol, "CONNECTING", false, std::nullopt,
                          "Connecting to live market data.");
          } else {
            notify_status(target.symbol, "ERROR", false,
                          queue_position_impl(target.symbol),
                          "Unable to request live market data.");
            transition_target_.reset();
          }
        } else {
  ...
  if (transition_target_) {
    ...
    if (target.subscribed && !target.subscribe_pending) {
      ...
      if (queue_needed(target)) {
        waiting_.erase(target_symbol);
        notify_status(target_symbol, "LIVE", true, std::nullopt,
  ...
  for (const auto &symbol : newly_live) {
    ...
    const auto state = states_.find(symbol);
    if (state != states_.end() && queue_needed(state->second)) {
      notify_status(symbol, "LIVE", true, std::nullopt,
                    "Live market data connected.");
    }
  }
...
void SubscriptionManager::handle_connection_changed_impl(bool connected) {
  ...
  eviction_victim_.reset();
  for (auto &[symbol, state] : states_) {
    const bool was_allocated = state.subscribed || state.subscribe_pending ||
    ...
    if (!state.subscribe_pending && queue_needed(state))
      update_waiting_entry(state);
  }
  notify_queue_positions();
}
...
std::size_t SubscriptionManager::active_subscription_count_impl() const {
  return static_cast<std::size_t>(
      std::count_if(states_.begin(), states_.end(), [](const auto &item) {
        const auto &state = item.second;
        return state.subscribed || state.subscribe_pending ||
               state.unsubscribe_pending;
      }));
}

std::optional<std::string>
SubscriptionManager::eviction_candidate_impl() const {
  const auto now = clock_();
  const SymbolSubscription *oldest = nullptr;
  for (const auto &[symbol, state] : states_) {
    static_cast<void>(symbol);
    if (!evictable(state, now))
      continue;
    if (oldest == nullptr ||
        state.last_requested_at < oldest->last_requested_at)
      oldest = &state;
  }
  return oldest == nullptr ? std::nullopt
                           : std::optional<std::string>(oldest->symbol);
}

bool SubscriptionManager::evictable(
    const SymbolSubscription &state,
    std::chrono::steady_clock::time_point now) const {
  return state.subscribed && !state.pinned && state.viewer_count == 0 &&
         state.pending_order_count == 0 && !state.subscribe_pending &&
         !state.unsubscribe_pending &&
...
void SubscriptionManager::notify_status(
    const std::string &symbol, const std::string &status, bool live,
    std::optional<std::size_t> queue_position,
    const std::string &message) const {
  if (status_handler_)
    status_handler_(symbol, status, live, queue_position, message);
}
