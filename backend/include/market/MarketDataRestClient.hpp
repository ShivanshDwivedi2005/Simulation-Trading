...
class MarketDataRestClient {
 public:
  ...
  [[nodiscard]] std::vector<nlohmann::json> latest_snapshots(const std::vector<std::string>& symbols);
  [[nodiscard]] nlohmann::json historical_bars(const std::string& symbol, std::size_t limit = 200);
...
[[nodiscard]] bool market_data_timestamp_is_fresh(
