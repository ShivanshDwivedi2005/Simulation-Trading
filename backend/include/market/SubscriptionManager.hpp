...
public:
  ...
  using StatusHandler = std::function<void(
      ...
      std::optional<std::size_t> queue_position, const std::string &message)>;
...
private:
  ...
  template <typename Result, typename Fn> Result invoke(Fn &&fn) const {
  ...
  template <typename Fn> void invoke_void(Fn &&fn) const {
  ...
  void handle_subscription_confirmation_impl(
  ...
  queue_position_impl(const std::string &symbol) const;
  void notify_queue_positions();
  void notify_status(const std::string &symbol, const std::string &status,
                     bool live, std::optional<std::size_t> queue_position,
                     const std::string &message) const;

  std::size_t active_subscription_count_impl() const;
  std::optional<std::string> eviction_candidate_impl() const;
  bool evictable(const SymbolSubscription &state,
